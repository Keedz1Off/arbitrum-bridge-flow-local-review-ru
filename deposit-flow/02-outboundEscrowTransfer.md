...
Эта функция is responsible for moving or locking the user's tokens on L1 before the bridge creates or finalizes the L2 credit.
...
```text
Bridge must credit L2 only for the amount that was actually received or escrowed on L1.
```
...
```text
Amount used downstream should match the real received / escrowed amount.
```
...
```text
Token being escrowed must be the intended L1 token.
```
